...
其更容易扩展。在本节中，我们将创建一个名为game_functions的新模块
它将存储大量让游戏
...
import sys
import pygame
from waixingren.bullet import Bullet
...
def check_keydown_events(event, ai_settings, screen, ship, bullets):
    ...
    elif event.key == pygame.K_q:   # q快捷键关闭程序
        sys.exit()
...
def check_events(ai_settings, screen, stats, sb, play_button, ship, aliens, bullets):     # 检查事件
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()
        elif event.type == pygame.KEYDOWN:    # 便在Pygame 检测到KEYDOWN事件时作出响应
